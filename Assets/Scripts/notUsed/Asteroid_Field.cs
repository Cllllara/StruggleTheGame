...
public class Asteroid_Field : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            randomAsteroid[i] = Random.Range(0, 4);
           ...
           asteroidClones[i] = Instantiate(asteroids[randomAsteroid[i]], new Vector3(transform.position.x + Random.Range(-spawnRange.x, spawnRange.x),
                                                                                     transform.position.x + Random.Range(-spawnRange.y, spawnRange.y),
                                                                                     transform.position.x + Random.Range(-spawnRange.z, spawnRange.z)), Quaternion.identity);
